...
describe('ZFilterSerialize', () => {
  ...
  const assertSerializes = (expected: string | undefined, candidate: IZFilter | null | undefined) => {
    // Arrange.
  ...
  describe('Binary', () => {
    const assertSerializesWithOperator = (operator: ZOperatorBinary) => {
      ...
      const filter = new ZFilterBinaryBuilder().subject('subject').value(100).operator(operator).build();
  ...
  describe('Unary', () => {
    const assertSerializesWithOperator = (operator: ZOperatorUnary) => {
      ...
      const filter = new ZFilterUnaryBuilder().subject('subject').operator(operator).build();
      assertSerializes(expected, filter);
    };
  ...
  describe('Collection', () => {
    const assertSerializesWithOperator = (operator: ZOperatorCollection) => {
      ...
      const filter = new ZFilterCollectionBuilder().subject('subject').values(values).operator(operator).build();
      assertSerializes(expected, filter);
    };
  ...
  describe('Logic', () => {
    const assertSerializesWithOperator = (operator: ZOperatorLogic) => {
      ...
      const a = new ZFilterUnaryBuilder().subject('subject').isNotNull().build();
      const b = new ZFilterBinaryBuilder().subject('subject').like().value('a').build();
      const filter = new ZFilterLogicBuilder().operator(operator).clause(a).clause(b).build();
      assertSerializes(expected, filter);
    };
